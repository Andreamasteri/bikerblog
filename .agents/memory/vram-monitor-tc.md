---
name: VRAM monitor on TC (AI Hub)
...
- The actual sampler runs on TC inside `ai-hub/server.js` (external repo, not this workspace; service formerly "horus-hub"): polls `nvidia-smi` on an interval, keeps a 24h rolling window persisted to a local JSON file (survives pm2 restarts), and exposes it via a gated `GET /vram` endpoint (current usage, 24h peak, best-effort per-process breakdown paired against `ollama ps` model names via a hardcoded model→agent map, overridable through an env var).
