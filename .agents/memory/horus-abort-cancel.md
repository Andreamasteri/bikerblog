...
- The web chat route creates one `AbortController` per request, tied to `res.on("close")` (NOT `req.on("close")`) — this fires both for an explicit client abort (Stop button cancels the fetch) and for tab-close/navigation, so no separate cleanup path is needed. `req.on("close")` (IncomingMessage) is a trap here: it fires almost immediately once `express.json()` finishes consuming the request body, long before the client actually disconnects, which silently aborted every SSE chat before the first token was ever sent. `res.on("close")` (ServerResponse) only fires on a genuine connection close.
