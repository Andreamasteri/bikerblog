...
**Known gap (2026-07-02):** having a working Ollama tunnel does NOT mean every other self-hosted service behind the same box is reachable the same way. This project's SearXNG instance on TC returns a plain nginx 403 (not a Cloudflare Access login redirect) on every path including `/`, while Ollama on the same box works fine through its own tunnel hostname — confirmed the failure is server-side (likely an nginx IP allowlist or missing Cloudflare Tunnel route for that hostname), not a header/auth issue on the calling script's side. Don't assume one self-hosted service's tunnel config generalizes to a sibling service; verify each hostname independently before debugging the client code.
