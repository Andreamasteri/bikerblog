...
**Status (resolved 2026-07-02):** fully deployed and verified end-to-end (health check + a real `git-log` call returned actual repo output). `HORUS_ANALYSIS_URL` + `ANALYSIS_GATE_TOKEN` are set; Horus now has real code analysis in both the web chat and CLI.

**Deployment gotcha hit along the way:** the standalone TC service reads secrets from `.env` via plain `process.env`, with no `dotenv` dependency — so `pm2 start server.js` alone does NOT pick up `.env` values, causing silent 401s (gate token appears "wrong" when it's actually just missing). Fix: start it with `pm2 start server.js --node-args="--env-file=.env"` (Node ≥20.6 built-in flag) instead of adding a dependency.

**How to apply:** for any future standalone Node service meant to run under pm2 outside this monorepo, either bundle a `.env` loader (env-file flag or `dotenv`) explicitly, or document that the process manager must be told to load `.env` itself — plain `node server.js` / `pm2 start server.js` will NOT read a sibling `.env` file automatically.
