...
**Generalized to N agents (done):** the two-agent union is gone. Agents now come from an ordered registry (`buildConvoAgentRegistry(deps)` → `ConvoAgentConfig[]` with `id`/`displayName`/`chatRaw`/`chatOptions`/`toolsNote`/`isConfigured`/`notConfiguredMessage`), turn = `agents[i % agents.length]`, and the system prompt takes `{selfName, otherNames[], previousSpeakerName, toolsNote}`. Output stays byte-identical for 2 agents. Adding a 3rd interlocutor (e.g. "Quebracho") = add one registry entry, no logic change. The DB `HorusConversationTurn.agent` was widened from the literal union to `string` to allow this. `deps.buildAgentRegistry?` overrides the registry (tests inject a 3-agent one to prove rotation/resume; see the "three agents" tests in the bowie-conversation test file). The frontend `horus-chat.tsx` still keeps its own `"horus" | "bowie"` client type — deliberately out of scope for the server structural refactor.
