...
**Status (2026-07-07): ATTIVO.** `HORUS_ANALYSIS_URL=https://analysis.biker-link.net` + `ANALYSIS_GATE_TOKEN` (secret) impostati su Replit. Il servizio gira su TC via pm2 (ecosystem.config.js in `/home/andrea/horus-analysis/`), porta 4600, esposto via CF tunnel `analysis.biker-link.net → http://127.0.0.1:4600` (ingress già configurato nel tunnel `bikerlink-tc`). SonarQube è attivo su TC (java su porta 9201/9300), ma `/capabilities` non è implementato nel server.js attuale → `sonar_scan` rimane nascosto (fail-closed corretto). Il servizio non ha CF Access app — usa solo l'header `X-Analysis-Gate-Token`.
