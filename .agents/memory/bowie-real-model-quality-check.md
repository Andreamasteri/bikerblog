...
**GPU persistence mode made boot-persistent (2026-07-04):** Ubuntu's packaged `nvidia-persistenced.service` runs with `--no-persistence-mode` (it does NOT itself enable persistence mode), so a manual `sudo nvidia-smi -pm 1` is transient and resets on reboot. Fixed by adding a custom oneshot unit `/etc/systemd/system/nvidia-persistence-mode.service` (`ExecStart=/usr/bin/nvidia-smi -pm 1`, `RemainAfterExit=yes`, `WantedBy=multi-user.target`), enabled via `systemctl enable`. Verify with `systemctl is-enabled nvidia-persistence-mode.service` + `nvidia-smi --query-gpu=persistence_mode --format=csv,noheader` (expect `enabled` / `Enabled`). Note: `sudo` on TC requires an interactive password — the agent's non-interactive SSH session cannot run `sudo` commands itself; any privileged command on TC must be relayed to the user to run by hand.
