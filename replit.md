...
- `pnpm --filter @workspace/scripts run horus:sse-smoke` — smoke check **opt-in** (non fa parte di CI né della pipeline notturna) che colpisce DAVVERO `/horus/chat`, `/horus/bowie-chat` e `/horus/bowie-conversation` sull'api-server in esecuzione, attraverso il tunnel Cloudflare reale (nessun `chatRaw` finto come nel test sopra), e verifica che arrivi almeno un evento reale (token/heartbeat/turn_start) entro un timeout. Va lanciato mentre l'api-server gira davvero (è lì che le env var di Horus/Bowie sono presenti in modo affidabile — vedi Gotchas), tipicamente da console mentre il workflow è attivo. Si auto-salta (exit 0) se `HORUS_CHAT_PASSWORD` o Horus non sono configurati; salta solo i check di Bowie se non è configurato lui. Env opzionali: `API_BASE_URL` (default `http://localhost:8080`), `HORUS_SMOKE_TIMEOUT_MS` (default 45000).
...
- **Analisi statica del codice (`typecheck_repo`, `lint_repo`, `search_code`, `git_log`, `sonar_scan`) gira su TC, non nel Repl**: `deploy/horus-analysis/` è un servizio Express standalone pensato per essere eseguito sull'hardware dell'utente (TC), che mantiene cloni git persistenti di bikerlink/bikerblog/bikerweb ed espone `/typecheck`, `/lint`, `/search`, `/git-log`, `/sonar`, `/capabilities` protetti da header `X-Analysis-Gate-Token`. `lib/horus/src/tools.ts` espone `getHorusTools()` (ora **async**) che aggiunge questi tool alla lista base SOLO se sono impostate entrambe le env var `HORUS_ANALYSIS_URL` e `ANALYSIS_GATE_TOKEN`; `HORUS_TOOLS` (deprecato) resta solo i tool base per compatibilità. Sia la web chat (`horus.ts`) sia la CLI (`horus-chat.ts`) usano `await getHorusTools()`, quindi anche Bowie (secondo agente sulla stessa lista) li vede. Setup del servizio su TC descritto in `deploy/horus-analysis/README.md`; l'agente Replit non ha accesso fisico a TC né può leggere il valore dei secret, quindi l'utente deve copiare token/URL manualmente.
- **`sonar_scan` aggiunge segnali SonarQube reali** (code smell, duplicazioni, vulnerabilità, security hotspot, debito tecnico) più profondi di `typecheck_repo`/`lint_repo`: lancia `sonar-scanner` sul clone del repo su TC, attende il Compute Engine task (analisi asincrona), poi interroga sia `/api/issues/search`+`/api/hotspots/search` sia `/api/measures/component` (metriche `duplicated_lines_density`, `sqale_index`, ecc.) e riassume tutto insieme. Richiede SonarQube Server (Docker, es. `sonarqube:2026-lta-community`) in esecuzione su TC, configurabile solo lato servizio (`SONARQUBE_URL`, `SONARQUBE_TOKEN` — mai visti dall'agente Replit). **Il tool è nascosto (non solo "fallisce con errore") se SonarQube non è configurato**: `lib/horus/src/tools.ts` interroga `GET /capabilities` sul servizio TC (cache 60s, timeout 3s, fail-closed) e toglie `sonar_scan` dalla lista se `sonarAvailable` è `false` o il servizio non risponde in tempo — è per questo che `getHorusTools()` è async. Se qualcuno lo invoca comunque durante una finestra di configurazione parziale, `/sonar` risponde con un messaggio chiaro invece di un errore generico. `architect` accetta un campo opzionale `extraContext` proprio per ricevere gli esiti di `sonar_scan` (o altri tool) senza rieseguire l'analisi — i tool "collaborano" in questo modo invece di restare isolati. Dettagli setup in `deploy/horus-analysis/README.md`.
