...
- **Ares — agente heavy on-demand (POWER)**: 4° agente NON residente in VRAM (modello pesante `devstral` via `ARES_OLLAMA_MODEL`). Su trigger admin sfratta la lineup residente, analizza UNA voce del backlog di supervisione e PROPONE ~2 percorsi di risoluzione, poi si scarica e ripristina la lineup (restore sempre nel `finally`). Invariante: "Ares propone, l'admin decide" — nessuna autocorrezione/auto-install. Trigger e lista sono admin-only (`/_internal/*`). Details: `.agents/memory/ares-heavy-agent.md`.
- **Coder pesante on-demand (Fase 2d power)**: riusa lo slot heavy di Ares (stesso `devstral`, stesso lock — `isCoderRunning()` è alias di `isAresRunning()`), ma con eviction **GATED sull'attività di chat**: non sfratta mai una sessione in corso. Gate: chat attiva → sempre rifiutato (anche admin); trigger non-admin (escalation Quebracho) → serve anche idle ≥ `CODER_MIN_IDLE_MS`. Ripristino lineup con rollback temporizzato (`CODER_RESTORE_TIMEOUT_MS`, default 60s) → alert che emerge in chat. Punto unico: `POST /_internal/coder/analyze`; stato condiviso: `GET /_internal/coder/status`; escalation: `pnpm --filter @workspace/scripts run coder:escalate`. Details: `.agents/memory/coder-heavy-on-demand.md`.
